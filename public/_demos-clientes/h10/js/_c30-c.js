...
function initModal_c30_c(order){
  ...
  $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"']").addClass("show");

  if(!$(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-slider").hasClass("slick-initialized")){
    $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-slider").on('beforeChange', function(event, slick, currentSlide, nextSlide){
      var slickActual = "";
      if(nextSlide < 9){ slickActual = "0"+(nextSlide+1);
      } else { slickActual = (nextSlide+1); }
      $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-contador .slick-actual").html(slickActual+"/ ");
    });

    $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-slider").on('init', function(event, slick){
      var slickActual = "";
      if(slick.currentSlide < 9){ slickActual = "0"+(slick.currentSlide+1);
      } else { slickActual = (slick.currentSlide+1); }
      $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-contador .slick-actual").html(slickActual+"/ ");
      $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-contador .slick-total").html("0"+slick.slideCount);
    });

    $(".c30-a__container.modal-detalle-habitacion[data-order='"+order+"'] .slick-slider-holder .slick-slider").slick({
      arrows: true,
      cssEase: 'ease',
      easing: 'easing',
      infinite: true,
